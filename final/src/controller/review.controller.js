...
export const createReview = async (req, res) => {
  ...
  if (!req.body) res.validationError("body 데이터를 입력해주세요!");
  if (!contents || !movieID)
    res.validationError("contents와 movieID 데이터를 입력해주세요!");
  if (!movieID < 0 || isNaN(movieID))
    res.validationError("ID 입력값이 양의 정수이어야 합니다!");
  ...
  res.success("댓글 등록 되었습니다!");
};

export const getReviewsById = async (req, res) => {
  const { movieID } = req.params || {};
  if (!movieID || isNaN(movieID))
    res.validationError("id가 유효하지 않습니다!");
...
export const deleteReview = async (req, res) => {
  ...
  if (!target) res.notFound(`${id} 댓글을 찾지 못하겠어요.`);
  ...
  return res.success(`${id} 댓글이 삭제되었습니다!`);
};
...
export const updateReview = async (req, res) => {
  ...
  if (contents.trim().length == 0)
    return res.validationError(
      "contents는 최소 한글자 있어야 합니다. (공백 없이)"
    );
